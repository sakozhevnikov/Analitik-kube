...
def show_data_preprocessing():
    ...
    # СОЗДАЕМ ДВЕ РАВНЫЕ КОЛОНКИ 50/50
    col_analysis, col_preprocessing = st.columns([1, 1])
    
    # ЛЕВАЯ КОЛОНКА - АНАЛИЗ ДАННЫХ (В КОНТЕЙНЕРЕ)
    with col_analysis:
        with st.container():
            ...
            # ОТОБРАЖЕНИЕ РЕЗУЛЬТАТОВ АНАЛИЗА В ЭТОМ ЖЕ КОНТЕЙНЕРЕ
            if st.session_state.get("show_preview", False):
                show_data_preview()
    ...
    # ПРАВАЯ КОЛОНКА - ПРЕДОБРАБОТКА ДАННЫХ (В КОНТЕЙНЕРЕ)
    with col_preprocessing:
        with st.container():
            st.title("Предобработка")
            
            # Здесь будут кнопки для предобработки данных
            st.info("Функции предобработки будут добавлены позже")
            
            # МЕСТО ДЛЯ БУДУЩИХ КНОПОК ПРЕДОБРАБОТКИ
            # st.button("Анализ пропусков", use_container_width=True)
            # st.button("Удаление дубликатов", use_container_width=True)
            # st.button("Изменение типов данных", use_container_width=True)
            # st.button("Фильтрация данных", use_container_width=True)
